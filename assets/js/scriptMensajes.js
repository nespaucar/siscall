...
$(document).ready(function() {
    ...
    $.ajax({
        url: '../controlador/contPersonal.php?accion=obtenerClientesReporte',
        type: 'GET',
        success: function(a) {
            $('#cboCliente').html(a);
            $('#cboCliente').chosen({
...
$(document).on('click', '.btnR', function(e) {
    ...
    var raizroute = '../reporte/repMensajes.php?accion=';
    var route = raizroute + 'reporteMensajes&tipo=';
    var type = 1;
    if ($(this).val() == 'btnPDF') {
        type = 2;
    ...
    if ($(this).attr('form') == '_dia') {
        if (dia) {
            ...
            if (hoy < fech1) {
                ...
                route += '&fecha1=' + dia + '&fecha2=' + dia + '&rango=DIARIO&idpersona=' + $("#cboCliente").val();
        ...
        if (hoy < fech) {
            ...
            route += '&fecha1=' + mano + '-' + mes + '-01&fecha2=' + mano + '-' + mes + '-31&rango=MENSUAL&idpersona=' + $("#cboCliente").val();
            success(type, route, corr);
        }
        ...
        if (anno < parseInt(ano)) {
            ...
            route += '&fecha1=' + ano + '-01-01&fecha2=' + ano + '-12-31&rango=ANUAL&idpersona=' + $("#cboCliente").val();
            success(type, route, corr);
        }
        ...
        if (dias_entre(df, di) > 0) {
            ...
            if (fech > hoy) {
                ...
                route += '&fecha1=' + di + '&fecha2=' + df + '&rango=DIARIO&idpersona=' + $("#cboCliente").val();
                success(type, route, corr);
            }
        } else {
            $('#di').focus();
        ...
        if (hoy < fech1) {
            ...
            if ((parseInt(amf) - parseInt(ami) > 0) || (parseInt(amf) - parseInt(ami) == 0 && parseInt(mf) - parseInt(mi) > 0)) {
                route += '&fecha1=' + ami + '-' + mi + '-01&fecha2=' + amf + '-' + mf + '-31&rango=MENSUAL&idpersona=' + $("#cboCliente").val();
                success(type, route, corr);
            } else {
                $('#mensajeReporte').css('color', 'red').html(incorr);
            }
        ...
        if (anno < parseInt(ai)) {
            ...
            if (parseInt(af) - parseInt(ai) > 0) {
                route += '&fecha1=' + ai + '-01-01&fecha2=' + af + '-12-31&rango=ANUAL&idpersona=' + $("#cboCliente").val();
                success(type, route, corr);
            } else {
                $('#mensajeReporte').css('color', 'red').html(incorr);
            }
